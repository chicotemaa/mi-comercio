...
const builtins: WebsiteMedia[] = [
  ...new Set([
    ...
    ...defaultWebsite.gallery.photos.map((p) => p.image),
    ...defaultWebsite.brand.photos.map((p) => p.image),
  ]),
].map((url, i) => ({
  id: `original-${i}`,
  url,
  name:
    i === 0
      ...
      : i === 1
        ? "Retrato de Nerea"
        : [
            ...defaultWebsite.gallery.photos,
            ...defaultWebsite.brand.photos,
          ].find((p) => p.image === url)?.title || "Nerea en el estudio",
  mime: "image/webp",
  size: 0,
  createdAt: "",
}));
...
const templateFor = (key: string): Value =>
  key === "items"
    ...
    : key === "posts"
      ? {
          ...defaultWebsite.journal.posts[0],
          id: crypto.randomUUID(),
          title: "Nueva nota",
          excerpt: "",
          content: "",
      ...
      : key === "photos"
        ? {
            ...defaultWebsite.gallery.photos[0],
            id: crypto.randomUUID(),
          ...
          <button
            ...
            onClick={() => {
              const item =
                path[0] === "brand"
                  ? {
                      image: defaultWebsite.brand.photos[0].image,
                      title: "Nueva imagen",
                      alt: "",
                    }
                  : templateFor(key);
