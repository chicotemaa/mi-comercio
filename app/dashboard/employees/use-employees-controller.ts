...
import type { EmployeeFeedbackState, EmployeeFormState, EmployeeServiceOption, EmployeeSummary } from "./employee-types"
import { createEmployeeFormState } from "./employee-utils"

function createFeedbackState(title: string, description: string, tone: EmployeeFeedbackState["tone"]) {
  return {
    title,
    description,
    tone,
...
export function useEmployeesController(employees: EmployeeSummary[], serviceOptions: EmployeeServiceOption[]) {
  const router = useRouter()
  const [isRefreshing, startTransition] = useTransition()
  const [searchTerm, setSearchTerm] = useState("")
  const [statusFilter, setStatusFilter] = useState<"all" | "active" | "inactive">("all")
  const [editingEmployee, setEditingEmployee] = useState<EmployeeSummary | null>(null)
  const [employeeToToggle, setEmployeeToToggle] = useState<EmployeeSummary | null>(null)
  const [formState, setFormState] = useState<EmployeeFormState>(createEmployeeFormState())
  const [isFormOpen, setIsFormOpen] = useState(false)
  const [isSubmitting, setIsSubmitting] = useState(false)
  const [isTogglingStatus, setIsTogglingStatus] = useState(false)
  const [formError, setFormError] = useState<string | null>(null)
  const [feedbackState, setFeedbackState] = useState<EmployeeFeedbackState | null>(null)
  ...
  function updateFormField<K extends keyof EmployeeFormState>(field: K, value: EmployeeFormState[K]) {
  ...
  function updateWorkingHour(dayOfWeek: number, field: "startTime" | "endTime" | "isActive", value: string | boolean) {
    setFormState((current) => ({
      ...current,
  ...
  async function submitForm() {
    ...
    const endpoint = editingEmployee ? `/api/employees/${editingEmployee.id}` : "/api/employees"
    ...
    try {
      ...
      setFeedbackState(
        createFeedbackState(
          editingEmployee ? "Profesional actualizado" : "Profesional creado",
          editingEmployee
  ...
  async function confirmToggleStatus() {
    ...
    try {
      const response = await fetch(`/api/employees/${employeeToToggle.id}/status`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      if (!response.ok) {
        ...
        setFeedbackState(
          createFeedbackState(
            ...
            body?.error ?? "Ocurrió un problema al cambiar el estado del profesional.",
            "error",
          ),
        )
        return
