import { createDefaultCategoryRateMap, formatCurrency } from "@/lib/business-shared"
...
export default async function EmployeesPage() {
  const { business, services, staffMembers, appointments, staffTimeLogs, staffWorkingHours, staffCategoryRates, staffServiceAssignments, isLive } =
  ...
  const employeeSummaries = staffMembers.map((staffMember) => {
    ...
    const completedAppointments = relatedAppointments.filter((appointment) => appointment.status === "completed")
    const activeAppointments = relatedAppointments.filter((appointment) => appointment.status !== "cancelled")
    const generatedRevenue = activeAppointments.reduce((total, appointment) => total + appointment.price, 0)
    const averageTicket = activeAppointments.length > 0 ? generatedRevenue / activeAppointments.length : 0
    ...
    const workingHours = staffWorkingHours
      ...
      .map((workingHour) => ({
        ...
        label: defaultWorkingHours.find((day) => day.dayOfWeek === workingHour.dayOfWeek)?.label ?? `Día ${workingHour.dayOfWeek}`,
    ...
    for (const rate of staffCategoryRates.filter((entry) => entry.staffMemberId === staffMember.id)) {
    ...
    const estimatedCompensation =
      staffMember.compensationType === "category_percentage"
        ? completedAppointments.reduce((total, appointment) => {
            const appointmentCategory = appointment.serviceId ? serviceMap.get(appointment.serviceId)?.category : null
            const percentage = appointmentCategory ? categoryRates[appointmentCategory] ?? 0 : 0
    ...
    return {
      ...
      assignedServiceIds,
      workingHours: workingHours.length > 0 ? workingHours : defaultWorkingHours,
      categoryRates,
